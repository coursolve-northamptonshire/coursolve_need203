...
class DataFetcher():
    '''Helper class to navigate & fetch the static datasets from the
    Northamptonshire County Council's data distribution website at:
        http://www.northamptonshireanalysis.co.uk.
    The data seems to be organized under the following hierarchy:
        - Themes : Broad themes such as "Population & Census',
        'Economy' etc.
        - SubThemes : Themes within each broad theme (e.g.
            Population/Age/Gender data within 'Population & Census'
        - Dataviews : The actual datasets, each subtheme could have
        multiple dataviews
        - Geo Ids : The geographical scope of the dataview -- could be
        county, dictrict etc.
    '''

    _ROOT_URL = 'http://www.northamptonshireanalysis.co.uk/'
    ...
    _RE_THEME = re.compile(r'^.*fetchChildObjects\(\'DataViews\',event,\'theme_(\d+)\'.*$')

    _RE_VIEW_URL = re.compile(r'^\./view\?viewId\=(\d+)$')
    ...
    def get_dataview_url(self, dataview_id, geo_name):
        args = urllib.urlencode({'viewId': dataview_id,
                                 'geoId': self._GEO_IDS[geo_name],
                                 'viewer': 'CSV',
                                 })
        csv_url = self._ROOT_URL + 'data/csv?' + args
    ...
    def fetch_dataviews(self, theme_id, dataview_links):
        for link in dataview_links:
            ...
            matchobj = self._RE_VIEW_URL.match(url)
    ...
    def fetch_subtheme(self, subtheme_name, subtheme_id):
        ''' The subtheme page contains a list of links to dataviews,
        parse them from the HTML
            e.g.
        'http://www.northamptonshireanalysis.co.uk/dataviews/rawlist?themeId=XXX'
        '''
        ...
        # Make the full URL
        url = self._ROOT_URL + 'dataviews/rawlist?' + args
        ...
        dvgen = self.fetch_dataviews

        return {'id': subtheme_id,
                'name': subtheme_name,
                'dataviews': [d for d in dvgen(subtheme_id,
                                               dataview_links)]}
        ...
        ''' algorithm:
         For each link:
             if it contains some embedded text:
                 ...
                 The 'onclick' attribute has some javascript that
                   contains the theme id, use a regex to extract it.
        '''
    ...
    def fetch_theme(self, theme_name):
        ...
        # Make the full URL
        url = self._ROOT_URL + 'bytheme?' + args
        ...
        # Find the 'scriptableTreeNode div element that contains the subthemes
        subtheme_parent = soup.find('div',
                                    {'class': 'scriptableTreeNode'})

        # Find all the 'a' element tags -- these contain the actual
        # subtheme descriptions
        subtheme_links = subtheme_parent.find_all('a')

        return {'name': theme_name,
                'id': self.theme_ids[theme_name],
                'subthemes' : [s for s in self.fetch_subthemes(theme_name, subtheme_links)] }
    ...
    def fetch_metadata(self):
        ...
        metadata = {
                        'geo_ids': [{'name': name,
                                     'id': self._GEO_IDS[name]}
                                    for name in self._GEO_IDS],
    ...
    def download_dataview_csv(self,
                              dir_name,
                              dv_id,
                              dv_desc,
                              geo_name,
        ...
        csv_file_name = 'dataview' + '_' + \
                        str(dv_id) + '_' + geo_name + '.csv'
        csv_file_name = os.path.join(dir_name,
                                     csv_file_name)
        if verbose:
            print("\t->  " + csv_file_name)
    ...
    def download_dataviews(self, dir_name, geo_name, verbose=False):
        for (dv_id, dv_desc) in self.get_dataview_ids_with_details().items():
            ...
            self.download_dataview_csv(dir_name,
                                       dv_id,
                                       dv_desc,
                                       geo_name,
                                       verbose)

    def download_all_dataviews(self,
                               dir_name,
                               verbose=False):
